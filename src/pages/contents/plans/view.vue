<template>
  <el-main class="content aaa111">
    <!-- 查看任務头部 -->
    <el-header class="header font-24">
      <img src="../../../assets/images/task/t_view.svg">
      {{ $t('plan.view.title') }}
    ...
    <el-row type="flex" class="infos" tag="section">
      <el-col tag="ul">
        <li class="info-item d-f">
          <p class="label">
            <img src="../../../assets/images/task/t_info.svg">
            {{ $t('plan.plan_name') }}：
          </p>
        ...
        <li class="info-item d-f des">
          ...
          <p
            class="text"
          >{{ $store.state.planInfo.description?$store.state.planInfo.description:$t('plan.plan_desc_no') }}</p>
        </li>
        <li class="info-item d-f">
          <p class="label">
            <img src="../../../assets/images/task/t_time.svg">
            {{ $t('plan.plan_createtime') }}：
          </p>
          <p class="text">{{ timeDeal($store.state.planInfo.created_at) }}</p>
        </li>
        <li class="info-item d-f">
          <p class="label">
            <img src="../../../assets/images/task/t_drone.svg">
            {{ $t('plan.plan_air') }}：
          </p>
        ...
        <li class="info-item d-f">
          <p class="label">
            <img src="../../../assets/images/task/t_frequency.svg">
            {{ $t('plan.plan_cycle') }}：
          </p>
        ...
        <li class="info-item d-f">
          <p class="label">
            <img src="../../../assets/images/task/t_first.svg">
            {{ $t('plan.plan_first_time') }}：
          </p>
          <p
            class="text"
          >{{ $store.state.planInfo.start_time?timeDeal($store.state.planInfo.start_time):$t('common.none') }}</p>
        </li>
        <li class="info-item d-f file">
          <p class="label">
            <img src="../../../assets/images/task/t_file.svg">
            {{ $t('plan.plan_mapfile') }}：
          </p>
          <p class="text">
            <el-button
              ...
              :disabled="!$store.state.planInfo.map_path"
    ...
    <el-header class="header font-24">
      <img src="../../../assets/images/task/t_history.svg">
      {{ $t('plan.view.history') }}
    </el-header>
    <!-- 任务执行历史 -->
    <el-table
      ...
      <el-table-column align="center" :label="$t('plan.view.raw_data')">
        <template slot-scope="scope">
          <el-button
            ...
            @click="downloadFile(scope.row.raw_data, 'raw_data')"
      ...
      <el-table-column align="center" :label="$t('plan.view.auto_run')">
        <template slot-scope="scope">
          <el-button
            ...
            @click="downloadFile(scope.row.orthomosaic_path, 'orthomosaic.tif')"
          >{{ $t('common.view') }}</el-button>
        </template>
      </el-table-column>
      <el-table-column align="center" border :label="$t('plan.view.logs')">
        <template slot-scope="scope">
          <el-button
            ...
            @click="downloadFile(scope.row.air_log_path, 'air_log.bin')"
          >{{ $t('common.download') }}</el-button>
        </template>
      </el-table-column>
      <el-table-column align="center" :label="$t('plan.view.sever_logs')">
        <template slot-scope="scope">
          <el-button
            ...
            @click="downloadFile(scope.row.sever_log_path, 'sever_log.bin')"
          >{{ $t('common.download') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <section v-if="showCheck" class="flip-mask pos-f">
      <section class="flip-wrapper d-ib va-m over-h text-l">
        <h4 class="pos-r title">
          {{ $t('plan.fly_check') }}
          <a
            class="pos-a close el-icon-close"
            @click.prevent="cancelRunPlan"
            href="javascript:;"
          ></a>
        </h4>
        <div class="content">
          ...
          <el-row class="body" type="flex" tag="div">
            <el-col :span="8" tag="ul">
              <li class="label weather">
                <img src="../../../assets/images/task/t_weather.svg">
                {{ $t('plan.weather') }}
              </li>
              <li class="label">
                <img src="../../../assets/images/task/t_drone.svg">
              ...
              <li class="label">
                <img src="../../../assets/images/task/t_airport.svg">
                {{ $t('common.depot') }}
              </li>
            </el-col>
          ...
          <div class="foot">
            ...
            <div class="foot-btns clear">
              <el-button
                class="f-l"
                @click.prevent="runPlan"
                type="danger"
              >{{ $t('plan.start_now') }}</el-button>
...
export default {
  ...
  methods: {
    ...
    hCells(cells) {
      return (
        "font-weight:normal;" +
        (cells.columnIndex === 3 ? "border-left: 1px solid #e4eaef;" : "")
      );
    },
    editTask() {
    ...
    cycleTypes(id) {
      switch (+id) {
        case 0:
          return this.$t("plan.edit.cycle_type_1");
        case 1:
          return this.$t("plan.edit.cycle_type_2");
        case 2:
          return this.$t("plan.edit.cycle_type_3");
        case 3:
          return this.$t("plan.edit.cycle_type_4");
        case 4:
          return this.$t("plan.edit.cycle_type_5");
        case 5:
          return this.$t("plan.edit.cycle_type_6");
      }
    },
    airDepot(id, type) {
      let name = "";
    ...
    runPlan() {
      let url =
        this.$store.state.config.suffix !== ""
          ? `${this.$store.state.api.local.plans}/${
              this.$store.state.planInfo.id
            }/plan_logs` + this.$store.state.config.suffix
          : `${this.$store.state.api.local.plans}/${
              this.$store.state.planInfo.id
            }/plan_logs`;
    ...
    downloadFile(path, name) {
      this.$http
        .get(this.$store.state.config.server + path, {
        ...
        .then(res => {
          ...
          a.download = name;
          try {
            a.download = contentDisposition.parse(
...
.flip-wrapper .body li.weather {
  height: 95px;
}
.flip-wrapper .body li.label {
  padding-left: 40px;
}
